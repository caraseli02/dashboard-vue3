...
type Docref =
  TypeOfFirebase.firestore.DocumentReference<TypeOfFirebase.firestore.DocumentData>;
/** Firestore CollectionReference */
type CollectionRef =
  TypeOfFirebase.firestore.CollectionReference<TypeOfFirebase.firestore.DocumentData>;
...
export default function <T, M = T>(options: Options<T, M>): any {
  ...
  if (!TypeOfFirebase.apps.length) {
    throw new Error(
      "vue-3-firestore Error: No default firebase app found. Please initialize firebase before calling useFirestore"
    );
  }
  ...
  const initialLoading =
    options.initialLoading === undefined ? true : options.initialLoading;
  ...
  const getCollData = async () => {
    try {
      ...
      return receiveCollData(
        collection.size ? collection.docs.map((x) => <T>x.data()) : []
      );
    } catch (e) {
      if (options.onError) {
        options.onError(e);
